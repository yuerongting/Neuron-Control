...
if(IDNI == 1)
    ...
    %         Total_time=8;  %10 ms
        ...
        for i=1:numel(t)-1
            ...
            time_in_current_period = mod(  time  ,  T_period )  ;
    ...
    % delta_u = inv([1,0,0,0] * subs(B_0,{u_e},{u_0})  ) * ( r_d - [1,0,0,0]*(  x_dot_0 ) - lambda_1*(r - hh ) - lambda_2 * (r-hh)^3);
        ...
        % %     if((V(i)> interval) || V(i)<0)  %% Negative current
            ...
            %% Model Dynamics 
            % I(i+1) = 0; %% test for 20% uncertainty: No current input, but system is unstable
...
%             display(find( V > -55, 1 ) *  deltaT );
            ...
            if j>1
                ...
                plot(t, V_record(j,:), marker_set(1), 'LineWidth',line_width, 'color', color_map(j,:));
                yyaxis right
                plot(t, I_record(j,:), marker_set(3), 'LineWidth',line_width, 'color',color_map(j,:));hold on
            else
                yyaxis left
                plot(t, V_record(j,:),marker_set(1),'LineWidth',line_width, 'color', color_map(j,:));hold on
                yyaxis right
                plot(t, I_record(j,:),marker_set(3),'LineWidth',line_width, 'color',color_map(j,:))
...
% 
% % Calculate the maximum and minimum values for each trajectory (row)
% max_values = max(I_record, 2);
% min_values = min(I_record, 2);
% 
% % Calculate the interval between max and min for each trajectory
% interval = max_values - min_values;
% 
% % Create a plot
% figure;
% 
% % Plot the shadowed confidence interval
% x = 1:j;
% y1 = min_values;
% y2 = max_values;
% fill([x, fliplr(x)], [y1', fliplr(y2')], 'b', 'EdgeColor', 'none', 'FaceAlpha', 0.3);
% hold on;
% 
% % Plot the interval line
% plot(x, interval, 'b-o');  % Change the line style/color as needed
% 
% xlabel('Trajectory');
% ylabel('Interval (Max - Min)');
% title('Interval Between Max and Min Values with Confidence Interval');
% grid on;
