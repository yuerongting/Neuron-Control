...
% lambda_1_range = [5, 1,  10];
% lambda_2_range = [5, 1,  10];
...
if(IDNI == 1)
    ...
    V_diff = V_rest - V_m;
    i=1;
    alpha_n = .01 * ( (10-V_diff(i)) / (exp((10-V_diff(i))/10)-1) );
    beta_n = .125*exp(-V_diff(i)/80);
    alpha_m = .1*( (25-V_diff(i)) / (exp((25-V_diff(i))/10)-1) );
    beta_m = 4*exp(-V_diff(i)/18);
    alpha_h = .07*exp(-V_diff(i)/20);
    beta_h = 1/(exp((30-V_diff(i))/10)+1);
    ...
    %     {V(i), n(i), m(i), h(i),      alpha_n, alpha_m, alpha_h,   beta_n, beta_m,   beta_h,    u_0});
    % 
    ...
    %         Total_time=8;  %10 ms
        ...
        ekf.State = double(x_0); %% Initial state 
        % ekf.StateCovariance = eye(4); %% Initial covariance 
        
        ekf.StateCovariance = diag(cov_scale); %% Initial covariance 
        ...
        for i=1:numel(t)-1
            ...
            r_d = ref * exp( -1 * (i)*deltaT);
            r_d_record(i) = r_d;

        %     track(i) = y_des_dot - ref;

            track(i) = ref * (1 - exp( -1 *(i)*deltaT)) + V_rest;
            ...
            first_nonzero_index = find(record_t ~= 0, 1, 'first');%% Time when reaching the threshold
...
% delta_u = inv([1,0,0,0] * subs(B_0,{u_e},{u_0})  ) * ( r_d - [1,0,0,0]*(  x_dot_0 ) - lambda_1*(r - hh ) - lambda_2 * (r-hh)^3);
        ...
        %     if((i>1)&& (V(i)<V(i-1)) ||(V(i)> interval) || V(i)<0) %% Positive current
            ...
            if (~isempty(first_nonzero_index))
                ...
                if( (V(i)> interval) || V(i)<0 || I(i+1) < 0 || i > first_nonzero_index) %% Positive current
...
%             display(find( V > -55, 1 ) *  deltaT );
            ...
            if j>1
                ...
                yyaxis right
                plot(cat(2,t_start,t),cat(2,I_start,I_record(1,:)), marker_set(3), 'LineWidth',line_width, 'color',color_map(j,:),'DisplayName',strcat(' Current Injection ', '\lambda_1=',num2str(-lambda_1_range(j)), ' ,\lambda_2=',num2str(-lambda_2_range(j))))
            else
                yyaxis left
                plot(cat(2,t_start,t),cat(2,V_start,V_record(1,:)),marker_set(1),'LineWidth',line_width, 'color', color_map(j,:), 'DisplayName',strcat(' Membrane Potential Voltage ','\lambda_1=',num2str(-lambda_1_range(j)), ' ,\lambda_2=',num2str(-lambda_2_range(j))));hold on
                yyaxis right
                plot(cat(2,t_start,t),cat(2,I_start,I_record(1,:)),marker_set(3),'LineWidth',line_width, 'color',color_map(j,:),'DisplayName',strcat(' Current Injection ','\lambda_1=',num2str(-lambda_1_range(j)), ' ,\lambda_2=',num2str(-lambda_2_range(j))))
